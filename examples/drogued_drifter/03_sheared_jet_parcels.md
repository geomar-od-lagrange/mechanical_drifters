...
We use `make_kernel(dd)` from `mechanical_drifters.parcels` to advect particles using the drogued drifter model. At each Parcels timestep, the kernel:
...
1. **Drogued drifter**: `make_kernel(dd)` on the standard FieldSet (the kernel extracts velocity profiles and computes drift internally).
...
```python
# Drogued drifter run — make_kernel(dd)
